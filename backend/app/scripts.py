...
def _validate(name: str, shell: str, content: str) -> None:
    if not name.strip():
...
async def create(name: str, shell: str, content: str, updated_by: str) -> dict[str, Any]:
    _validate(name, shell, content)
    async with _connect() as db:
        cur = await db.execute(
            "INSERT INTO scripts (name, shell, content, updated_by, updated_at)"
            " VALUES (?, ?, ?, ?, ?)",
            (name.strip()[:120], shell, content, updated_by, int(time.time())),
        )
        await db.commit()
...
async def update(
    script_id: int, name: str, shell: str, content: str, updated_by: str
) -> dict[str, Any] | None:
    _validate(name, shell, content)
    async with _connect() as db:
        await db.execute(
            "UPDATE scripts SET name = ?, shell = ?, content = ?, updated_by = ?, updated_at = ?"
            " WHERE id = ?",
            (name.strip()[:120], shell, content, updated_by, int(time.time()), script_id),
        )
        await db.commit()
